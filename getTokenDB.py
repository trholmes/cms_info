...
def check_credentials(cfg, args):
    ...
    secret still work. Pass --token/--token-file to examine one obtained
    elsewhere instead.
    """
    supplied = read_supplied_token(args)
    if supplied:
        token, audience = supplied, decode_claims(supplied).get('aud')
        print(f'examining the supplied token (audience "{audience}")')
    else:
